 ...
 * Laboratório 03 - Matrizes esparsas                            */
...
void inicializa(matriz *a, int m, int n) {
    ...
    for (j = 0; j < n; j++) {
		...
		t->ac = t;
		t->ab = t;
		/* Insere logo a esquerda do super-cabeça: */
		t->dir = r;
		t->esq = r->esq;
		t->dir->esq = t;
		t->esq->dir = t;
...
void libera(matriz *a) {
  /* Libera toda a memória dinâmica ocupada por uma matriz */
	...
	for (i = 0; i <= a->nlins; i++) {
		...
		/* Se não houver nenhum elemento na linha já libera a memoria do no cabeca dela */
		if (t == t->dir) { 
			FREE(t);
		} else {
			t = t->dir;
	...
	/* Liberas as memorias dos vetores de apontadores para os nos cabeça e da matriz */
	FREE(a->clin);
	FREE(a->ccol);
	FREE(a);
}
